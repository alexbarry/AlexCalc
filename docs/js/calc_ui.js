...
var TokenType;
(function (TokenType) {
    TokenType[TokenType["DIGIT"] = 0] = "DIGIT";
    TokenType[TokenType["VAR"] = 1] = "VAR";
    TokenType[TokenType["FUNC_CALL"] = 2] = "FUNC_CALL";
    TokenType[TokenType["PAREN_OPEN"] = 3] = "PAREN_OPEN";
    TokenType[TokenType["PAREN_CLOSE"] = 4] = "PAREN_CLOSE";
    TokenType[TokenType["OP"] = 5] = "OP";
    TokenType[TokenType["OTHER"] = 6] = "OTHER";
})(TokenType || (TokenType = {}));
;
...
function cap_cursor_idx(ui) {
    let len = ui.state.input_tokens.length;
    if (ui.state.cursor_idx < 0) {
        ui.state.cursor_idx = 0;
    }
    if (ui.state.cursor_idx >= len) {
        ui.state.cursor_idx = len;
    }
}
/*
...
function input_tokens_eq(a, b) {
    return a.str == b.str && a.is_unit == b.is_unit;
}
function input_token_arrays_eq(a, b) {
    if (!a || !b) {
        return false;
    }
...
function history_entry_add(ui, input_tokens) {
    ...
    console.debug("history update {pos:", ui.state.input_history_pos, ", history: ", ui.state.input_history, "}");
}
...
function handle_btn_history(ui, change) {
    ...
    console.debug("handle_btn_history: pos is now ", ui.state.input_history_pos, ", input is now: ", new_input);
