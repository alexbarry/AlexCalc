let g_callback_switch_to_custom_colours = undefined;
...
function get_css_rule(selectorText) {
...
function padStart(val, n, pad) {
    while (val.length < n) {
        val = pad + val;
    }
    return val;
}
function rgba_to_hex(rgb_str) {
...
function load_existing_colours() {
    for (const { label, colour_sel_id } of COLOUR_SEL_INFO) {
        const colour_sel = document.getElementById(colour_sel_id);
        if (!colour_sel) {
        ...
        if (has_fg.has(label)) {
            ...
            const colour_sel_fg = document.getElementById(colour_sel_fg_id);
            if (!colour_sel_fg) {
...
function apply_colours() {
    for (const info of COLOUR_SEL_INFO) {
        ...
        if (has_fg.has(label)) {
            ...
            const colour_sel_fg = document.getElementById(colour_sel_id + "_fg");
...
export function set_callback_switch_to_custom_colours(callback) {
...
export function set_colour_adjuster_pane_visible(is_visible) {
